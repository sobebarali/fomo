...
> left column + header. Reads BirdEye through [`../../integrations/birdeye`](../../integrations/birdeye)
...
- **Errors:** `RATE_LIMITED` (BirdEye free-tier limit) · `UPSTREAM_ERROR` (BirdEye down/malformed).
...
| Rule | Why |
|------|------|
| All market data comes from the `birdeye` integration (cached there), never `fetch` in the handler. | One cache + rate-limiter; free-tier safe; the handler stays a thin map (root domain rules). |
...
| Map a BirdEye 429 → `RATE_LIMITED`, any other failure/malformed payload → `UPSTREAM_ERROR`. | Stable codes for the client + tests; never leak a raw 500. |
...
- **Calls:** `birdeye.trending()`, `birdeye.token(address)` ([`../../integrations/birdeye`](../../integrations/birdeye)).
...
The list/detail must reflect **live** BirdEye data (no hardcoded tokens), yet a free-tier rate-limit
or upstream blip must surface as `RATE_LIMITED`/`UPSTREAM_ERROR` — never a fake row and never a 500.
The integration test mocks the BirdEye client to assert: happy path shape, 429 → `RATE_LIMITED`,
malformed payload → `UPSTREAM_ERROR`, invalid mint → `BAD_REQUEST`, stable cursor paging.

## Links

Integration: [`../../integrations/birdeye/AGENTS.md`](../../integrations/birdeye/AGENTS.md) ·
