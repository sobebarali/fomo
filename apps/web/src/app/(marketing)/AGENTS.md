...
Rotating token banner (top) → nav + **Sign in (Apple/Google via Privy)** → hero (headline + app-store
badges + phone mock) → features grid → product showcase → CTA → rotating token banner (bottom) → footer.
...
- Brand tokens (colors/fonts) come from `@fomo/ui` `globals.css`; app-store badges + mobile links from `TASK.md`.
